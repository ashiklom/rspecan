...
############################################################
metadata <- alldat %>%
  left_join(plots %>% setDT()) %>%
  setDF() %>%
  as_tibble() %>%
  rename(
    observation_id = samplecode,
  ...
  mutate_at(
    vars(matches("leaf_.*_per_area")),
    ~add_metadata(., data_unit = "ug cm-2")
  ) %>%
  add_metadata(
    project_code = project_code,
    short_name = "Di Vittorio 2009",
    long_name = "Di Vittorio 2009",
    spectra_methods = list(
      comment = "see the paper"
    )
  )

spec_df <- spectra_raw %>%
  mutate(col_name = paste(samplecode, spectratype, sep = "||")) %>%
  select(-spectratype, -samplecode) %>%
  spread(col_name, spectravalue)
...
cn <- colnames(spec_mat)
obs_ids <- str_remove(cn, "\\|\\|.*")
st <- str_remove(cn, ".*\\|\\|") %>%
  recode(reflectance = "R", transmittance = "T")
colnames(spec_mat) <- obs_ids
spectra <- spectra(spec_mat, waves, st)
